...
void SetSimpleData(ty::KeyFrameList& list) {
	list.InsertKey(2, 0.f, ty::EasingType::Linear);
	list.InsertKey(5, 5.f, ty::EasingType::Exponential);
	list.InsertKey(10, 10.f, ty::EasingType::Smoothstep);
}
void CheckSimpleData(ty::KeyFrameList& list) {
	{
		...
		EXPECT_EQ(p.mStart.mEasingType, ty::EasingType::Zero);
	}
	{
	...
	}
	{
		...
		EXPECT_EQ(p.mStart.mEasingType, ty::EasingType::Linear);
	}
	{
	...
	}
	{
		...
		EXPECT_EQ(p.mStart.mEasingType, ty::EasingType::Exponential);
	}
	{
	...
	}
	{
		...
		EXPECT_EQ(p.mEnd.mTick, 10);
		EXPECT_EQ(p.mStart.mEasingType, ty::EasingType::Smoothstep);
	}
	{
